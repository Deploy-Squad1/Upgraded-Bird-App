...
from . import app, db
from .models import Banned_IP, Image, User
...
def check_ip_allowed():
    ...
    if ip:
        ...
        if banned_ip:
            return redirect(
                "https://protocol.ua/ua/kriminalniy_kodeks_ukraini_stattya_248/"
            )
    return None
...
def reveal_location(image_id):
    ...
    if image.password_hash and check_password_hash(
        image.password_hash, password_attempt
    ):
        flash(f"Location: {image.location}", "success")
    else:
